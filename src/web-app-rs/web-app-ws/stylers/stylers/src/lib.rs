...
pub fn build(output_path: Option<String>) {
            ...
            if let Item::Fn(fn_def) = item {
                ...
                for stmt in fn_def.block.stmts {
                    ...
                    if let Stmt::Local(let_bin) = stmt
                        && let Some(init) = let_bin.init
                        && let Expr::Macro(expr_mac) = init.expr.borrow()
                        && let Some(path_seg) = expr_mac.mac.path.segments.last()
                    {
    ...
    write_css(output_path, &output_css).unwrap_or_else(|e| p!("Problem creating output file: {}", e.to_string()));

    p!("{}", "===============================Stylers debug output end===============================");
}

pub fn build2(glob_root: String, output_path: Option<String>) {
    let pattern = format!("{}/src/**/*.rs", glob_root);
    let mut output_css = String::from("");
    p!("{}", "===============================Stylers debug output start===============================");
    for file in glob(&pattern).unwrap() {
        let file = file.unwrap();
