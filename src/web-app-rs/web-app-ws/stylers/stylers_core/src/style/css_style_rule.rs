...
impl StyleRule {
    ...
    pub(crate) fn new(ts: TokenStream, class: &Class, is_proc_macro: bool) -> (StyleRule, HashSet<String>) {
            ...
            match tt {
                TokenTree::Group(t) => {
                    ...
                    if t.delimiter() == Delimiter::Brace {
                        ...
                        add_spaces(&mut selector, t.span(), &mut pre_line, &mut pre_col, is_proc_macro);
    ...
    pub(crate) fn parse_selector(&mut self, selector_text: &str, class: &Class) -> HashSet<String> {
        ...
        for c in selector_text.chars() {
            ...
            if c == ':'
                && let Some(sub) = selector_text.get(i..i + 4)
                && sub == "deep"
            {
