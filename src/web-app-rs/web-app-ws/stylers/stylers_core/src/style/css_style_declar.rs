...
impl StyleDeclaration {
    pub(crate) fn new(group: Group, is_proc_macro: bool) -> StyleDeclaration {
        let mut css_style_declar = StyleDeclaration { style_css_text: "".to_string() };
    ...
    pub(crate) fn parse(&mut self, group: Group, is_proc_macro: bool) {
        ...
        group.stream().into_iter().for_each(|tt| match tt {
            TokenTree::Group(t) => {
                add_spaces(&mut property, t.span(), &mut pre_line, &mut pre_col, is_proc_macro);
            ...
            TokenTree::Literal(t) => {
                ...
                property.push_str(t.to_string().trim_start_matches('r').trim_matches(|c| c == '#' || c == '"'));
            }
            TokenTree::Punct(t) => {
                let ch = t.as_char();
                ...
                if is_property_start && ch == ':' {
                    ...
                    if !is_valid {
                        panic!("Did you mean to use {} property at line number {}", suggest.expect("Expected suggestion"), pre_line);
                    }
                }
...
fn parse_property_group(group: Group, raw_str: bool, is_proc_macro: bool) -> String {
    ...
    group.stream().into_iter().for_each(|tt| match tt {
        TokenTree::Group(t) => {
            add_spaces(&mut body, t.span(), &mut pre_line, &mut pre_col, is_proc_macro);
